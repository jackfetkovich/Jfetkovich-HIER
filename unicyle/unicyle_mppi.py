...
params = Parameters(
    ...
    max_w_dot = 0.4, # max angular acceleration (radians/s^2) (8.0)
...
def main():
    ...
    main_safety_ratio = int((params.dt*100) / (params.safety_dt*100))
    ### Zeroed arrays used for calcuation
    Tx = int(distance_of_path(np.array(points)) / (params.max_v*0.5*params.safety_dt))    ## Generation of waypoints for obstacle and robot
